...
public interface SeatHoldRepository extends JpaRepository<SeatHold, UUID> {
    ...
    List<SeatHold> findByShowIdAndStatusAndExpiresAtAfter(UUID showId, SeatHoldStatus status, LocalDateTime expiresAt);
    ...
    List<SeatHold> findByShowIdAndSeatIdAndStatusAndExpiresAtAfter(UUID showId, UUID seatId, SeatHoldStatus status, LocalDateTime expiresAt);
